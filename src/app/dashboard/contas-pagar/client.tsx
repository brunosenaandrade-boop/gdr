...
export function ContasPagarClient({ transactions, categories, tenantId }: Props) {
  ...
  return (
    <>
      <AppHeader title="Contas a Pagar" description="Despesas pendentes e vencidas">
      ...
      <div className="p-6 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <Card className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs text-slate-500 uppercase tracking-wider">A Vencer</p>
              <p className="text-lg font-semibold text-amber-400 tabular-nums">{formatCurrency(totalPendentes)}</p>
            </div>
          </Card>
          <Card className="flex items-center gap-4">
        ...
        <Card className="p-0 overflow-hidden">
          {transactions.length === 0 ? (
            <div className="py-20 text-center text-sm text-slate-500">Nenhuma conta a pagar pendente</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
